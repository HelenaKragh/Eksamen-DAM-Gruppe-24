...
=======
>>>>>>> b964727ff37c1c90ca6e4bbf96faadd1164c2381
...
library(leaflet)
library(readxl)
Sogn_Koordinater_sidste <- read_excel("data/Sogn_Koordinater_sidste.xlsx", 
                                      col_types = c("text", "numeric", "numeric"))
View(Sogn_Koordinater_sidste)
...
4) Derudover kan vi se i vores dataframe, at sognet "Gylling" stod usundt i cellen for longitute ift. resten af værdierne i kolonnen. Derfor vil vi lokalisere cellen for at rette fejlen. 

```{r}
Koordinater[28,][[3]] <- 10.1620478
```
...
Koordinater <- Koordinater %>% 
  mutate(content_popup= paste0("<h1>", sogn, "</h1> Her kan stå noget om sognets fattige"))
```
...
Koordinater %>% 
  ...
  addMarkers(~longitude,~latitude,
             popup = ~content_popup)
```
7) Fejl i visualiseringen: Koordinaterne der er indsamlet manuelt fra Google Maps er ved få tilfælde blevet taget fra det forkerte sogn (Se vestjylland). Dette er en menneskelig fejlkilde, der ville kunne forbedres ved en dybere grundighed. 
#################################################
=======
...
# ret årshus købstad til stort k
```{r}
...
```
>>>>>>> ba666976493a8645c01a8ae696bf16d04912d4cd
